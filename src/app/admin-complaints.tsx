...
export default function AdminComplaints() {
  ...
  const [activeMainTab, setActiveMainTab] = useState<'complaint' | 'suggestion'>('complaint');
  ...
  const handleReply = async (ticket: any) => {
    ...
    try {
      // 1. حفظ الرد في الشكوى نفسها
      await updateDoc(doc(db, 'support_tickets', ticket.id), {
        reply: text.trim(),
        status: 'replied'
      });
      
      // 2. 👈 إرسال إشعار داخلي يظهر في جرس إشعارات التطبيق
      await addDoc(collection(db, 'notifications'), {
        userId: ticket.captainId || ticket.passengerId, 
        userType: ticket.userType || (ticket.captainId ? 'captain' : 'passenger'),
        title: 'رد من الإدارة 📩',
        message: `تم الرد على ${ticket.type === 'suggestion' ? 'مقترحك' : 'شكواك'}:\n"${text.trim()}"`,
        timestamp: serverTimestamp(),
        read: false,
        sender: 'الإدارة'
      });

      // 3. إرسال إشعار موبايل Push
      await sendPushNotification(ticket.pushToken, 'رد جديد من الإدارة 📩', 'تم الرد على رسالتك، تفقد سجل الشكاوى.');
  ...
  const filteredTickets = tickets.filter(ticket => {
    ...
    const matchType = activeMainTab === 'suggestion' ? isSuggestion : !isSuggestion;
  ...
  const renderTicket = ({ item }: { item: any }) => (
    <View style={styles.card}>
      <View style={styles.cardHeader}>
        <Text style={styles.typeBadge}>{item.type === 'suggestion' ? '💡 مقترح' : '⚠️ شكوى'}</Text>
    ...
    <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined} style={styles.container}>
      ...
      <View style={styles.mainTabsContainer}>
        <TouchableOpacity style={[styles.mainTab, activeMainTab === 'complaint' && styles.mainTabActiveComplaint]} onPress={() => setActiveMainTab('complaint')}>
          <Text style={[styles.mainTabText, activeMainTab === 'complaint' && styles.mainTabTextActive]}>الشكاوى ⚠️</Text>
        </TouchableOpacity>
        <TouchableOpacity style={[styles.mainTab, activeMainTab === 'suggestion' && styles.mainTabActiveSuggestion]} onPress={() => setActiveMainTab('suggestion')}>
          <Text style={[styles.mainTabText, activeMainTab === 'suggestion' && styles.mainTabTextActive]}>المقترحات 💡</Text>
        </TouchableOpacity>
      </View>
...
const styles = StyleSheet.create({
  ...
  headerTitle: { fontSize: 18, fontWeight: 'bold', color: '#eab308' },
  ...
  mainTabsContainer: { flexDirection: 'row-reverse', gap: 10, marginBottom: 15 },
  mainTab: { flex: 1, paddingVertical: 12, borderRadius: 12, alignItems: 'center', backgroundColor: '#1e293b', borderWidth: 1, borderColor: '#334155' },
  ...
  mainTabText: { fontSize: 15, fontWeight: 'bold', color: '#94a3b8' },
  ...
  typeBadge: { fontWeight: 'bold', color: '#0f172a', backgroundColor: '#eab308', paddingHorizontal: 10, paddingVertical: 4, borderRadius: 6, fontSize: 12 },
